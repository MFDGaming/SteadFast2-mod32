...
//		$this->setDataProperty(self::DATA_SHOW_NAMETAG, self::DATA_TYPE_BYTE, $value ? 1 : 0);
	...
	public function getDataProperty($id){
		return isset($this->dataProperties[$id]) ? $this->dataProperties[$id][1] : "0";
	}
	...
	public function setDataFlag($propertyId, $id, $value = true, $type = self::DATA_TYPE_LONG, $send = true){
		if($this->getDataFlag($propertyId, $id) !== $value){
			...
			$flags = BCBitwise::bitwiseXor($flags,  BCBitwise::leftShift("1", (string) $id));
	...
	public function getDataFlag($propertyId, $id){
		return bccomp(BCBitwise::bitwiseAnd((string) $this->getDataProperty($propertyId) ?? "0", BCBitwise::leftShift("1", (string) $id)), "0") == 1;
	}
